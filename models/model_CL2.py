...
class WNBAPerformancePredictor:
    def __init__(self):
        self.data_files = {
    ...
    def load_player_data(self, player_name):
        ...
        try:
            for year, file_path in self.data_files.items():
                try:
            ...
            if all_data:
                ...
                self.player_data = self.player_data.sort_values('game_date').reset_index(drop=True)

                # Calculate field goal percentage
                self.player_data['fg_pct'] = np.where(
                    self.player_data['field_goals_attempted'] > 0,
                    self.player_data['field_goals_made'] / self.player_data['field_goals_attempted'],
                    0
                )

                # Remove rows with NaN values in critical columns
                critical_columns = ['points', 'field_goals_made', 'field_goals_attempted', 'game_date']
                self.player_data = self.player_data.dropna(subset=critical_columns)

                # If after cleaning we have no data, return False
    ...
    def calculate_rolling_averages(self, window=5):
        """Calculate rolling averages for performance metrics"""
        if self.player_data is None:
            return

        self.player_data['points_rolling'] = self.player_data['points'].rolling(window=window, min_periods=1).mean()
        self.player_data['fg_pct_rolling'] = self.player_data['fg_pct'].rolling(window=window, min_periods=1).mean()
        self.player_data['minutes_rolling'] = self.player_data['minutes'].rolling(window=window, min_periods=1).mean()

    def identify_performance_dips(self):
        """Identify games where player significantly underperformed"""
        if self.player_data is None:
            return

        # Calculate z-scores for key metrics
        self.player_data['points_zscore'] = (
                (self.player_data['points'] - self.player_data['points'].mean()) /
                self.player_data['points'].std()
        )

        self.player_data['fg_pct_zscore'] = (
                (self.player_data['fg_pct'] - self.player_data['fg_pct'].mean()) /
                self.player_data['fg_pct'].std()
        )

        # Combined performance score (lower is worse)
        self.player_data['performance_score'] = (
                                                        self.player_data['points_zscore'] + self.player_data[
                                                    'fg_pct_zscore']
                                                ) / 2

        # Identify significant dips (bottom 25%)
        dip_threshold = self.player_data['performance_score'].quantile(0.25)
        self.player_data['is_dip'] = self.player_data['performance_score'] < dip_threshold

    def find_cyclical_patterns(self):
        """Analyze for roughly monthly patterns in performance dips"""
        if self.player_data is None:
            return

        dip_games = self.player_data[self.player_data['is_dip'] == True].copy()
    ...
    def predict_next_dip_window(self):
        """Predict when the next performance dip might occur"""
        if not self.cycle_patterns:
            return None

        last_dip = self.cycle_patterns['last_dip_date']
        avg_cycle = self.cycle_patterns['average_cycle_days']
        cycle_std = self.cycle_patterns['cycle_std']
        ...
        return {
            ...
            'confidence': min(100, self.cycle_patterns['cycle_count'] * 20)  # Max 100%
        }

    def is_in_predicted_dip_window(self, game_date):
        """Check if a given date falls within a predicted performance dip window"""
        prediction = self.predict_next_dip_window()
        if not prediction:
            return False, 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        for player in props_data['players']:
            ...
            result = predictor.predict(player_info)
